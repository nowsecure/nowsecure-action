...
const types_1 = __nccwpck_require__(3374);
__exportStar(__nccwpck_require__(3374), exports);
const execLog = debug_1.default('ripgrep-js:exec');
...
function ripGrep(cwd, optionsOrSearchTerm) {
    ...
    return new Promise(function (resolve, reject) {
        child_process_1.exec(execString, { cwd }, (error, stdout, stderr) => {
...
class NowSecure {
     ...
     */
    pollForReport(reportId, pollInterval) {
        return __awaiter(this, void 0, void 0, function* () {
            let report = null;
            for (;;) {
                console.log("Checking for NowSecure report... ", reportId);
                report = yield this.pullReport(reportId);
...
function convertToSarif(data, keyParams = utils_1.DEFAULT_KEY_PARAMS, filter = utils_1.DEFAULT_SARIF_FILTER, labUrl = DEFAULT_LAB_URL, rainier = true) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        report.findings = report.findings.filter((finding) => (0, utils_1.findingMatchesFilter)(finding, filter));
        const rules = [];
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.submitSnapshotData = exports.convertToSnapshot = void 0;
 ...
 */
const snapshotUrl = (owner, repo) => `https://api.github.com/repos/${owner}/${repo}/dependency-graph/snapshots`;
 ...
 */
function convertToSnapshot(deputy, githubCorrelator, { ref, sha, job, runId }) {
...
exports.convertToSnapshot = convertToSnapshot;
/**
 ...
 */
function submitSnapshotData(data, { repo: { owner, repo } }, token) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (r.message.statusCode !== 201) {
            throw new Error(`Snapshot request failed with status ${r.message.statusCode}`);
        }
    });
}
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const details = yield ns.submitBin(fs_1.default.createReadStream(appFile), groupId, versionString, analysisType);
...
function outputToDependencies(report, context, githubCorrelator, githubToken) {
    var _a, _b, _c;
    return __awaiter(this, void 0, void 0, function* () {
        const deputy = (_c = (_b = (_a = report === null || report === void 0 ? void 0 : report.data) === null || _a === void 0 ? void 0 : _a.auto) === null || _b === void 0 ? void 0 : _b.assessments[0]) === null || _c === void 0 ? void 0 : _c.deputy;
        if (deputy) {
            console.log("Converting NowSecure report to Snapshot format...");
            const snapshotData = (0, nowsecure_snapshot_1.convertToSnapshot)(deputy, githubCorrelator, context);
            console.log("Uploading Snapshot data to GitHub...");
            return (0, nowsecure_snapshot_1.submitSnapshotData)(snapshotData, context, githubToken);
        }
        else {
            console.warn("NowSecure did not find dependencies information for this report");
        }
    });
}
...
exports.outputToSarif = outputToSarif;
...
function loadRawConfig(configPath) {
    ...
    if (stat.isDirectory()) {
        configPath = path_1.default.join(configPath, ".nsconfig.yml");
    }
 ...
 */
function checkObject(test, allowedKeys, what) {
    if (typeof test !== "object" || (0, lodash_1.isArray)(test)) {
        throw new TypeError(`${what} must be an object`);
    }
    for (const k in test) {
        if (allowedKeys.indexOf(k) < 0) {
            throw new errors_1.KeyError(`${k} is not permitted in ${what}`);
        }
    }
    return true;
}
...
class NsConfig {
     ...
     */
    constructor(configPath, checkIds = null) {
    ...
    parseFilters(rawConfig, checkIds = null) {
        ...
        if ("filters" in rawConfig) {
            ...
            if (typeof filters !== "object" || (0, lodash_1.isArray)(filters)) {
            ...
            for (const filterName in filters) {
                ...
                if (checkObject(filterConfig, FILTER_KEYS, "Filter")) {
                    this.filters[filterName] = (0, filter_1.parseFilter)(filterConfig, checkIds);
                }
            }
        }
    }
    parseConfigs(rawConfig) {
        if ("configs" in rawConfig) {
            const configs = rawConfig.configs;
            if (typeof configs !== "object" || (0, lodash_1.isArray)(configs)) {
    ...
    getRawFilter(filterName) {
        ...
        if (!(filterName in this.filters)) {
            throw new errors_1.ValueError(`Filter ${filterName} is not defined`);
        }
        return this.filters[filterName];
    }
    /** Return the full filter from the default any specified fields */
    mergeFilters(defaultFilter, update) {
        return Object.assign(Object.assign({}, (0, lodash_1.cloneDeep)(defaultFilter)), (0, lodash_1.cloneDeep)(update));
    }
    getConfig(configName, configType) {
        let raw = undefined;
        if (configName) {
            if (!(configName in this.configs)) {
                throw new errors_1.ValueError(`Config ${configName} is not defined`);
            }
...
function findingMatchesFilter(finding, filter) {
    ...
    if (filter.includeWarnings && ((_b = (_a = finding.check) === null || _a === void 0 ? void 0 : _a.issue) === null || _b === void 0 ? void 0 : _b.warn)) {
        return true;
    }
    return false;
}
exports.findingMatchesFilter = findingMatchesFilter;
/**
 * Severities to include in report based off user input
 */
function severityToSarif(input) {
 ...
 */
function isValidCheck(checkInputs, listName, checkIds = null) {
    ...
    if (checkIds) {
        const isValid = filteredChecks.every((check) => checkIds.includes(check.toLowerCase()));
        if (!isValid)
            throw new errors_1.ValueError(`The following checkId(s): [${filteredChecks.filter((check) => !checkIds.includes(check))}] within the ${listName} list are not valid.`);
    }
...
function parseFilter(filterConfig, checkIds = null) {
    ...
    if (severityInput !== undefined) {
        ...
        if (!config_types_1.severity.includes(severityInput.toLowerCase())) {
            throw new errors_1.ValueError(`${severityInput} is not a valid severity filter type`);
        }
    ...
    if (includeWarnings !== undefined) {
        if (typeof includeWarnings !== "boolean") {
            throw new TypeError("include-warnings must be a boolean value if specified");
        }
    ...
    if (checkedConfig.includeChecks && checkedConfig.excludeChecks) {
        ...
        if (combinedChecks.length)
            throw new InvalidFilterError(`Unique checkId(s) must be limited to either the exclude or include list. 
...
__exportStar(__nccwpck_require__(8545), exports);
...
/******/ 	var __webpack_exports__ = __nccwpck_require__(5507);
